...
//  Top Advertisement Image
   ...
   $wp_customize->add_control('madarlite_search_icon_in_menu', array(
    ...
    $wp_customize->add_control(
        ...
        array(
            'type' => 'checkbox',
            'label' => __('Show Recent Post Bloc', 'madar-lite'),
            'section' => 'header-settings',
            'priority' => 4,           
        )
    );
...
function madarlite_style_styles() {
	...
	?>
  <style type="text/css">
	  	.colour-line,.date-header,.main-navigation-menu li.current-menu-item,.single-header .comment,.single-header .comment,.home-box-header span,#back-top,a.custom-li,.cbp-qtprogress,.widget-header:after, .flex-control-paging li a.flex-active,.post-tags a,.footer-widget-header:after {background: <?php echo $theme_color;?>; }
