...
require get_template_directory() . '/widgets/slider-post.php';
if ( is_admin() ) {
...
/*-----------------------------------------------------------------------------------*/
	...
	add_image_size( 'madar-recent', 218, 360, true);
...
if ( ! function_exists( 'madarlite_fonts_url' ) ) :
	/**
	...
	function madarlite_fonts_url() {
	    ...
	    */
	    $open_sans = _x( 'on', 'Open Sans font: on or off', 'madar-lite' );

	    /* Translators: If there are characters in your language that are not
	    * supported by Oswald, translate this to 'off'. Do not translate
	    * into your own language.
	    */
	    $oswald = _x( 'on', 'Oswald font: on or off', 'madar-lite' );

	    if ( 'off' !== $oswald || 'off' !== $open_sans ) {
	        $font_families = array();

	        if ( 'off' !== $oswald ) {
	            $font_families[] = 'Oswald:400,500,600,700,300,100,800,900';
	        }
 ...
 */
if ( ! function_exists( 'madarlite_category_color' ) ) :
function madarlite_category_color( $wp_category_id ) {
   $args = array(
      'orderby' => 'id',
      'hide_empty' => 0
   );
   ...
   foreach ($category as $category_list ) {
      $color = get_theme_mod('madarlite_category_color_'.$wp_category_id);
      return $color;
   }
}
endif;
 ...
 */
function madarlite_colored_category_title($title) {
   $color_value = madarlite_category_color(get_cat_id($title));
   $color_border_value = madarlite_category_color(get_cat_id($title));
...
function madarlite_category_title_function($category_title) {
   add_filter('single_cat_title', 'madarlite_colored_category_title');
}
add_action('madarlite_category_title','madarlite_category_title_function');

if ( !function_exists('madarlite_colored_category') ) :
   function madarlite_colored_category() {
      global $post;
      ...
      if($categories) {
         ...
         foreach($categories as $category) {
            $color_code = madarlite_category_color(get_cat_id($category->cat_name));
            if (!empty($color_code)) {
               $output .= '<a href="'.get_category_link( $category->term_id ).'" style="background:' . madarlite_category_color(get_cat_id($category->cat_name)) . '" rel="category tag">'.$category->cat_name.'</a>'.$separator;
            } else {
...
function madarlite_nav_fallback(){
	echo '<a class="menu-fallback" href="' . admin_url('nav-menus.php') . '">' . __( 'Create your menu here', 'madar-lite' ) . '</a>';
}
/*-----------------------------------------------------------------------------------*/
...
function madarlite_thumbnail($pagename, $imgtype = '', $the_thumb=''){
	if($pagename == 'post'){
		...
		$size = array(185,110);
	}elseif($pagename == 'recent'){
		$the_thumb = 'madar-recent';
		$size = array(218,360);
	}
...
function madarlite_scripts() {
	...
	wp_enqueue_script( 'appslider', get_template_directory_uri() . '/js/app.js', false, '', true );
	wp_enqueue_script( 'search', get_template_directory_uri() . '/js/search.js', false, '', true );	
    wp_enqueue_style('madarlite-fonts', madarlite_fonts_url(), array());
